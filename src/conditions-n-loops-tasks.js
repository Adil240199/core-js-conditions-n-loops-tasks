...
function getSpiralMatrix(size) {
  ...
  while (startRow <= endRow && startCol <= endCol) {
    ...
    for (i = endCol; i >= startCol; i -= 1) {
      matrix[endRow][i] = counter;
      counter += 1;
    }
    endRow -= 1;

    for (i = endRow; i >= startRow; i -= 1) {
...
function getNearestBigger(number) {
  ...
  for (let i = digits.length - 2; i >= 0; i -= 1) {
    if (digits[i] < digits[i + 1]) {
      let minLargerIdx = i + 1;
      for (let j = i + 2; j < digits.length; j += 1) {
        if (digits[j] > digits[i] && digits[j] < digits[minLargerIdx]) {
          minLargerIdx = j;
        }
      }

      [digits[i], digits[minLargerIdx]] = [digits[minLargerIdx], digits[i]];
